...
class BasicConv(BaseModule):
    ...
    def init_weights(self):
        ...
        # torch.nn.init.constant_(self.conv.bias, bias_value)
...
class TripletAttention(BaseModule):
    ...
    def forward(self, x):
        # x:[8,128,64,64],[b,c,h,w]->[b,w,h,c]
        x_perm1 = x.permute(0, 3, 2, 1).contiguous()
        x_out1 = self.ChannelGateH(x_perm1)
        x_out11 = x_out1.permute(0, 3, 2, 1).contiguous()

        #x: [8, 128, 64, 64], [b, c, h, w]->[b, h, c, w]
        x_perm2 = x.permute(0, 2, 1, 3).contiguous()
        x_out2 = self.ChannelGateW(x_perm2)
        x_out21 = x_out2.permute(0, 2, 1, 3).contiguous()
